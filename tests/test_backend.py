...
def test_mcq_schema_validation():
    ...
    item = MCQItem(
        ...
        options=["5", "10", "12", "15"],
        correct_answer="10",
        explanation="Standard ODI rules limit each bowler to 10 overs.",
        grounding=grounding
    )
...
def test_guess_number_schema_validation():
    ...
    item = GuessTheNumberItem(
        ...
        sport="Basketball",
        difficulty="Medium",
        question="How many total points did Wilt Chamberlain score in his iconic 1962 game?",
        target_number=100.0,
        accepted_tolerance_range="±0",
        explanation="Wilt Chamberlain scored 100 points.",
        grounding=grounding
    )
...
def test_agent_schema_validation_function():
    valid_mcq = {
        ...
        "sport": "Tennis",
        "difficulty": "Hard",
        "question": "How many Roland Garros singles titles has Rafael Nadal won?",
        "options": ["10", "12", "14", "16"],
        "correct_answer": "14",
        "explanation": "Rafael Nadal won 14 French Open titles.",
    ...
    invalid_poll = {
        "id": "poll_1",
        "sport": "Tennis",
        "prompt": "Better forehand?",
        "options": ["Federer", "Nadal", "Djokovic"] # Invalid: 3 options instead of 2
    }
    invalid_validated = agent_engine.validate_and_parse_item(invalid_poll, "This-or-That Poll")
